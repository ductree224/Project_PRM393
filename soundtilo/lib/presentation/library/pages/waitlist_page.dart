...
class _WaitlistPageState extends State<WaitlistPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Danh sách chờ phát'),
        centerTitle: true,
      ),
      body: BlocBuilder<WaitlistBloc, WaitlistState>(
        builder: (context, waitlistState) {
          if (waitlistState is WaitlistLoading ||
              waitlistState is WaitlistInitial) {
              ...
              builder: (context, playerState) {
                ...
                if (currentPlayingId != null) {
                  final playingIndex = tracks.indexWhere(
                    (t) => t.externalId == currentPlayingId,
                  );
                  if (playingIndex != -1) {
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ReorderableListView.builder(
                        itemCount: tracks
                            .length, // Lấy toàn bộ danh sách thay vì chỉ lấy activeTracks
                        onReorder: (oldIndex, newIndex) {
                          ...
                          if (currentPlayingId != null) {
                            final newPlayingIndex = tempTracks.indexWhere(
                              (t) => t.externalId == currentPlayingId,
                            );
                          ...
                          context.read<WaitlistBloc>().add(
                            WaitlistReorderTracks(newOrderIds, newFadedCount),
                          );
                        },
                        itemBuilder: (context, index) {
                          ...
                          final isFaded =
                              index <
                              actualFadedCount; // Bài đứng trước là bài đã nghe -> Làm mờ
                          final isPlayingSlot =
                              index ==
                              actualFadedCount; // Bài ở ranh giới là bài đang phát
                          ...
                          if (isFaded) {
                            ...
                            if (isPlayerPlaying &&
                                track.externalId == currentPlayingId) {
                          ...
                          return Dismissible(
                            ...
                            onDismissed: (_) {
                              context.read<WaitlistBloc>().add(
                                WaitlistRemoveTrack(track.externalId),
                              );
                            },
                              ...
                              child: ListTile(
                                ...
                                onTap: () {
                                  if (isPlayingSlot) {
                                    ...
                                    final currentTrackIds = tracks
                                        .map((t) => t.externalId)
                                        .toList();
                                    context.read<WaitlistBloc>().add(
                                      WaitlistReorderTracks(
                                        currentTrackIds,
                                        index,
                                      ),
                                    );
                                ...
                                trailing: isFaded
                                    ...
                                    : const Icon(
                                        Icons.drag_handle,
                                        color: AppColors.grey,
                                      ), // Ẩn tay cầm kéo thả ở bài đã mờ
                              ),
                            ),
                          );
                        },
